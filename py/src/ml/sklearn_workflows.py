...
def run_logistic_regression(dataset: PreparedDataset) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "Logistic regression requires at least two target classes.")
...
def run_svm(dataset: PreparedDataset) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "SVM requires at least two target classes.")
...
def run_knn(dataset: PreparedDataset, k_neighbors: int = 3) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "KNN classification requires at least two target classes.")
...
def run_atom_neighbor_report(
    ...
    return {
        ...
        "neighbors": [
            {
                "atom": str(dataset.frame.iloc[int(index)].get("atomLabel", int(index))),
...
def run_decision_tree(dataset: PreparedDataset) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "Decision tree classification requires at least two target classes.")
    ...
    return {
        ...
        "feature_importances": {
            ...
            for feature, importance in zip(dataset.feature_columns, model.feature_importances_, strict=True)
        },
    }
...
def run_random_forest(dataset: PreparedDataset) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "Random forest classification requires at least two target classes.")
...
def run_hierarchical_clustering() -> dict[str, Any]:
    ...
    labeled_frame["cluster"] = labels
    class_breakdown = (
        labeled_frame.groupby(["cluster", "animalClass"]).size().rename("count").reset_index().to_dict("records")
    )
    ...
    return {
        ...
        "linkage_preview": linkage_matrix[: min(8, len(linkage_matrix))].astype(float).tolist(),
    }
...
def run_naive_bayes(dataset: PreparedDataset) -> dict[str, Any]:
    if not has_multiple_classes(dataset):
        return insufficient_class_result(dataset, "Naive Bayes classification requires at least two target classes.")
