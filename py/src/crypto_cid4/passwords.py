...
def password_dependencies_available() -> bool:
    return importlib.util.find_spec("argon2") is not None and importlib.util.find_spec("bcrypt") is not None
...
def build_password_hash_examples(password: str) -> dict[str, Any]:
    ...
    scrypt_hash = hashlib.scrypt(password_bytes, salt=scrypt_salt, n=2**14, r=8, p=1).hex()
    pbkdf2_hash = hashlib.pbkdf2_hmac("sha256", password_bytes, pbkdf2_salt, 600_000).hex()
    ...
    return {
        ...
        "algorithms": {
            ...
            "bcrypt": {
                "hash": bcrypt_hash,
                "verified": bool(bcrypt.checkpw(password_bytes, bcrypt_hash.encode("utf-8"))),
            },
            "scrypt": {
                "salt_b64": base64.b64encode(scrypt_salt).decode("ascii"),
                "hash": scrypt_hash,
                "verified": hashlib.scrypt(password_bytes, salt=scrypt_salt, n=2**14, r=8, p=1).hex() == scrypt_hash,
            },
            "pbkdf2_hmac_sha256": {
                "salt_b64": base64.b64encode(pbkdf2_salt).decode("ascii"),
                "iterations": 600_000,
                "hash": pbkdf2_hash,
                "verified": hashlib.pbkdf2_hmac("sha256", password_bytes, pbkdf2_salt, 600_000).hex() == pbkdf2_hash,
            },
        },
        "notes": [
