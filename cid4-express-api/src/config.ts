...
export async function resolveServerConfig(dataDir: string): Promise<ServerConfig> {
  const host = firstEnvValue("EXPRESS_HOST", "SERVER_HOST") ?? "0.0.0.0"
  const port = firstIntEnvValue("EXPRESS_PORT", "SERVER_PORT", "PORT") ?? 8443
...
async function resolveServerConfigFromCryptoSummary(
  ...
  if (!certPath || !keyPath) {
    throw new Error(`Express TLS fallback requires PEM paths in ${summaryPath}`)
  }
